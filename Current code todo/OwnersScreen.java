...
public class OwnersScreen {
	...
	public void runAdmin(Owners machine) throws IOException {
		...
		while (more) {
			...
			if (command.equals("1")) {
				...
				System.out.println(ownerPicked.getPropertiesArray().get(index - 1));
				// System.out.println(machine.getOwnersProperties());
			} else if (command.equals("2")) {
				/** Shows list of owners to pick */
				...
				int year = in.nextInt();
				System.out.println("Enter key eircode (leave blank if you want all of Ireland: ");
				String eircode = in.nextLine().toUpperCase();
				if (eircode.equals("")) {
					for (int i = 0; i < machine.getOwnerDetails().size(); i++) {
						for (int j = 0; j < machine.getOwnerDetails().get(i).getPropertiesArray().size(); j++) {
							machine.getOwnerDetails().get(i).getPropertiesArray().get(j);
						}
					}
				}
			} else if (command.equals("4")) {
	...
	public void SingleOwnerScreen(Owner ownerPicked) {
		...
		while (more2) {
			...
			if (propertyScreenChoice.equals("4")) {
			...
			} else if (propertyScreenChoice.equals("5")) {
			...
			} else if (propertyScreenChoice.equals("6")) {
				for (int i = 0; i < ownerPicked.getPropertiesArray().size(); i++) {
			...
			} else if (propertyScreenChoice.equals("7")) {
				more2 = false;

			} else if (propertyScreenChoice.equals("1")) {
				System.out.println(ownerPicked.getProperties());

			} else if (propertyScreenChoice.equals("2")) {
				System.out.println("Enter pick a property");
				for (int i = 0; i < ownerPicked.getPropertiesArray().size(); i++) {
					System.out.println((i + 1) + ") " + ownerPicked.getPropertiesArray().get(i).getAddress());
				}
				int choice = in.nextInt();
			...
			} else if (propertyScreenChoice.equals("4")) {
